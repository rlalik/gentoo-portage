...
inherit desktop linux-info linux-mod multilib-build optfeature \
	readme.gentoo-r1 toolchain-funcs unpacker
...
src_prepare() {
	...
	eapply "${FILESDIR}"/${P}-tirpc.patch

	gzip -d nvidia-{cuda-mps-control,modprobe,persistenced,smi}.1.gz || die
	...
	if use kernel_linux && kernel_is ge 5 4; then
		einfo "Patching files for kernels higher than 5.3"
	...
	if use kernel_linux && kernel_is ge 5 6; then
		...
		eapply ${FILESDIR}/${P}-kernel-5.6.patch
	fi

	if use kernel_linux && kernel_is ge 5 7; then
		einfo "Patching files for kernels higher than 5.6"
		eapply ${FILESDIR}/${P}-kernel-5.7.patch
	fi

	if use kernel_linux && kernel_is ge 5 8; then
		einfo "Patching files for kernels higher than 5.7"
		eapply ${FILESDIR}/${P}-kernel-5.8.patch
	fi

	if use kernel_linux && kernel_is ge 5 9; then
		einfo "Patching files for kernels higher than 5.8"
		eapply ${FILESDIR}/${P}-kernel-5.9.patch
	fi

	if use kernel_linux && kernel_is ge 5 10; then
		einfo "Patching files for kernels higher than 5.9"
		eapply ${FILESDIR}/${P}-kernel-5.10.patch
	fi

	if use kernel_linux && kernel_is ge 5 11; then
		einfo "Patching files for kernels higher than 5.10"
		eapply ${FILESDIR}/${P}-kernel-5.11.patch
		eapply ${FILESDIR}/kernel_5.11_compiled.patch
	fi

	if use kernel_linux && kernel_is ge 5 12; then
		einfo "Patching files for kernels higher than 5.11"
		eapply ${FILESDIR}/${P}-kernel-5.12.patch
	fi

	if use kernel_linux && kernel_is ge 5 13; then
		einfo "Patching files for kernels higher than 5.12"
		eapply ${FILESDIR}/${P}-kernel-5.13.patch
	fi

	if use kernel_linux && kernel_is ge 5 14; then
		einfo "Patching files for kernels higher than 5.13"
		eapply ${FILESDIR}/${P}-kernel-5.14.patch
	fi
